...
export default {
  ...
  computed: {
    ...
    summaries () {
      return [
        ...
        { type: 'icon', icon: 'ant-design:star-filled', text: 'Joining Project Grant 1000 Points' },
  ...
  methods: {
    async getDetails () {
      ...
      // .then(res => {
      //   if (this.project.project_team.members.filter(member => member.member_id === this.user.id).length === 1) {
      //     this.$router.back()
      //   }
      // })
    },
