<template>
  <div>
    <div class="mb-3" :class="{ 'shortlist-container': teams.length > 0 }">
      <ShortlistTeamItem v-for="(e, index) in teams" :key="`ShortlistIndividualItem-${e.id}`" :data="e" :index="index" @click="showDetails" />
...
import ShortlistTeamItem from '~/components/ShortlistTeamItem'

export default {
  name: 'ShortlistTeamPage',

  components: { ShortlistTeamItem },
